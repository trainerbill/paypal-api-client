...
export async function createAccessToken() {
  ...
  const res = await fetch(`${PAYPAL_HOSTNAME}/v1/oauth2/token`, options);
...
export async function createOrder(data?: any) {
  ...
  const res = await fetch(`${PAYPAL_HOSTNAME}/v2/checkout/orders`, options);

  return await res.json();
}

export async function authorizeOrder(id: string, data?: any) {
  const token = await getAccessToken();
  ...
  const res = await fetch(
    `${PAYPAL_HOSTNAME}/v2/checkout/orders/${id}/authorize`,
    options
  );
...
export async function loadPayPalCheckoutScript(
